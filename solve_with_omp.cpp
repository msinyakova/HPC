...
char* create_path(Process proc, int N, int core, int thread)
{
	char* path = new char[80];
	sprintf(path, "%s","solutionOMP");
	char str_thread[80];
	char str_n[80];
	char str_core[80];
	char str_file[80];
	sprintf(str_thread, "%d_L1_", thread);
	sprintf(str_n, "N%d_", N);
	sprintf(str_core, "c%d/", core);
	strcat(path, str_thread);
	strcat(path, str_n);
...
FILE* print_matrix(Matrix& arr, Process proc, int start[3], int N, int core, int thread, float inaccuracy[K])
{
	...
	char* path = create_path(proc, N, core, thread);
...
{
	int i, j, k;
	#pragma omp parallel for shared(u0, u1) private(i, j, k)
	//calculation inside points
    for(i = start[0] + 1; i < start[0] + u0.X() - 1; i++) {
    	for (j = start[1] + 1; j < start[1] + u0.Y() - 1; j++) {
    		for (k = start[2] + 1; k < start[2] + u0.Z() - 1; k++) {
    ...
    if (proc.down_n < 0) {
    	#pragma omp parallel for shared(u0, u1) private(i, j)
	    for (i = start[0]; i < start[0] + u0.X(); i++) {
	    	for (j = start[1]; j < start[1] + u0.Y(); j++) {
	...
	if (proc.up_n < 0) {
		#pragma omp parallel for shared(u0, u1) private(i, j)
	    for (i = start[0]; i < u0.X(); i++) {
	    	for (j = start[1]; j < u0.Y(); j++) {
	...
	if (proc.left_n < 0) {
		#pragma omp parallel for shared(u0, u1) private(i, k)
	    for (i = start[0]; i < start[0] + u0.X(); i++) {
	    	for (k = start[2]; k < start[2] + u0.Z(); k++) {
	...
	if (proc.back_n < 0) {
		#pragma omp parallel for shared(u0, u1) private(k, j)
	    for (j = start[1]; j < start[1] + u0.Y(); j++) {
	    	for (k = start[2]; k < start[2] + u0.Z(); k++) {
...
{ 
	//calculation inside points
	int i, j, k;
	#pragma omp parallel for shared(next_arr, current_arr, previos_arr) private(i, j, k)
	for(i = 1; i < next_arr.X() - 1; i++) {
    	for (j = 1; j < next_arr.Y() - 1; j++) {
    		for (k = 1; k < next_arr.Z() - 1; k++) {
...
int main(int argc, char** argv) {
	...
	if (argc < 4) {
		std::cout << "no parameter N or core number or thread number" << std::endl;
		MPI_Finalize();
		return 1;
	}
	int N;
	...
	FILE* fout = print_matrix(current_arr, proc, start, N, core, thread, inaccuracy);
